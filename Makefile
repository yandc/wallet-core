...
ifeq ($(OS),web)
	...
	LDFLAGS = -fexceptions -s LLD_REPORT_UNDEFINED -s EXPORTED_FUNCTIONS='["_CppUtxoPlan", "_CppPublicKeyWithMiliKey", "_CppAddressCreateWithMiliKey", "_CppAddressIsValid", "_CppDecodeEvmCall", "_CppSignMili23", "_CppJsonTransactionMili23", "_TWStringSize", "_TWStringUTF8Bytes", "_TWStringDelete", "_TWStringCreateWithUTF8Bytes", "_TWAnySignerSignJSON", "_TWDataCreateWithHexString", "_CppAddressCreateWithPKey"]' -s EXPORTED_RUNTIME_METHODS='["ccall","cwrap"]'
	...
	LDFLAGS = -fexceptions -L./build/lib/ -lgo_mili -lc++ -lm

else ifeq ($(OS),linux)
	CFLAGS += -fPIC
	TARGET ?= $(BIN_PATH)/wallet_core.$(OS).$(ARCH).so
	LDFLAGS = -fexceptions -L./build/lib/ -lgo_mili -lstdc++ -lm
endif
